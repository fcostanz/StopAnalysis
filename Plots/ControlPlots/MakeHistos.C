...
int MakeHistos(int iSample = 0){
  ...
  for ( int iDir = 0; iDir < NDirs; iDir++){
    controlDir[iDir] = outFile->mkdir( controlDirName[iDir]);
    controlDir[iDir]->cd();

    npvh[iDir] = new TH1D( "npv", "NPV", 51, -0.5, 50.5);
    ngoodpvh[iDir] = new TH1D( "ngoodpv", "NgoodPV", 51, -0.5, 50.5);

    lpth[iDir]     = new TH1D( "lpt", "lep p_{T} [GeV]",  25, 25., 500.);
    ...
    jet1h[iDir]  = new TH1D(  "jet1", "1st jet p_{T} [GeV]",   50, 0., 500.);
    jet2h[iDir]  = new TH1D(  "jet2", "2nd jet p_{T} [GeV]",   50, 0., 500.);
    jet3h[iDir]  = new TH1D(  "jet3", "3rd jet p_{T} [GeV]",   50, 0., 500.);
    jet4h[iDir]  = new TH1D(  "jet4", "4th jet p_{T} [GeV]",   50, 0., 500.);
    
    nbjetsh[iDir]    = new TH1D( "nbjets", "b jets multiplicity",  6, -0.5, 5.5);  
    bjet1h[iDir]     = new TH1D(     "bjet1", "Leading b jet p_{T} [GeV]", 25, 0., 500.);
    bjetHighDh[iDir] = new TH1D( "bjetHighD", "p_{T} of the highest b disc jet [GeV]",  25, 0., 500.);
    bdiscHh[iDir]    = new TH1D( "bdisc", "bdisc",  40, 0., 1.);  
    ...
    mth[iDir]      = new TH1D(  "Mt",  "Mt [GeV]", 25, 0., 500.);
  ...
  for (int ievt=0;ievt<N;++ievt){    
	...
	continue;
      ...
      if (iDir%NLeps == 1 || iDir%NLeps == 2 )
 	if (mt > 100.) {
          if (iSample != 0) mth[iDir]->Fill( mt, weight);
          continue;
        }
