  ...
  for (int ilep = 0; ilep < NLep; ilep++){
	...
	if (iSample == 1 || iSample == 2) {
	  if (wToph->GetBinContent(1) > 0.)
	    mth[iSample]->Scale( woToph->GetBinContent(1) / wToph->GetBinContent(1));
	}
	...
	peakPreh[iSample]->SetBinError(1, error);
      ...
      histoName = lep[ilep]; histoName += "-CR5/wTopPtReweight";	
      inBaseDir[1]->GetObject( histoName, wToph);
      inBaseDir[2]->GetObject( histoName, tmph);
      wToph->Add( tmph);     
      
      histoName = lep[ilep]; histoName += "-CR5/woTopPtReweight";	
      inBaseDir[1]->GetObject( histoName, woToph);
      inBaseDir[2]->GetObject( histoName, tmph);
      woToph->Add( tmph);      

      for (int iSample = 0; iSample < NSamples; iSample++){
	histoName = lep[ilep]; histoName += "-CR5/Mt";
	inBaseDir[iSample]->GetObject(histoName, mth[iSample]);
