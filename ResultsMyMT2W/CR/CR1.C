...
int CR1(){
  ...
  for (int ilep = 0; ilep < NLep; ilep++){
	...
	SFh[isf]->Reset();
      ...
      ////////////////////
      tmph->Clear();   tmph->Reset();
      wToph->Clear();  wToph->Reset();
      woToph->Clear(); woToph->Reset();
      TString histoName;
      for (int iSample = 0; iSample < NSamples; iSample++){
	histoName = lep[ilep]; histoName += "-CR1/Mt";
	...
	tailh[iSample]->SetBinError(1, error);
      ...
      histoName = lep[ilep]; histoName += "-CR1/wTopPtReweight";	
      inBaseDir[1]->GetObject( histoName, wToph);
      inBaseDir[2]->GetObject( histoName, tmph);
      wToph->Add( tmph);     
      
      histoName = lep[ilep]; histoName += "-CR1/woTopPtReweight";	
      inBaseDir[1]->GetObject( histoName, woToph);
      inBaseDir[2]->GetObject( histoName, tmph);
      woToph->Add( tmph);
      
      if (wToph->GetBinContent(1) > 0.){
	peakh[1]->Scale( woToph->GetBinContent(1) / wToph->GetBinContent(1));
	peakh[2]->Scale( woToph->GetBinContent(1) / wToph->GetBinContent(1));
	...
	tailh[2]->Scale( woToph->GetBinContent(1) / wToph->GetBinContent(1));
      ...
      peakSFh[3]->Clear(); peakSFh[3]->Reset();
