...
public:
  ...
  int CR4CR5Unc( Float_t& unc){    
    unc = srBkgCorrh[0]->GetBinContent(1) * 0.30;
  ...
  int RTopUnc( Float_t& unc){    
    unc = srBkgh[1]->GetBinContent(1) * RToph->GetBinError(1.) / RToph->GetBinContent(1.);
  ...
  int WSFUnc( Float_t& unc){    
    unc = (srBkgCorrh[1]->GetBinContent(1) + srBkgCorrh[2]->GetBinContent(1)) * 0.30;
  ...
  int WXSUnc( Float_t& unc){    
    ...
    WXSUp->Scale( 1.3, 2, bUp, eUp);

    Double_t bDown = 0.;
    Double_t eDown = 0.;
    BkgEstimation* WXSDown = new BkgEstimation( *this);
    WXSDown->Scale( 0.7, 2, bDown, eDown);
  ...
  int RareXSUnc( Float_t& unc){    
    ...
    RareXSDown->Scale( 0.5, 2, bDown, eDown);
  ...
  int TotUnc( Float_t& unc){
    ...
    this->NJetsModellingUnc( 1.20, NJetsModellingUnc);
...
int BkgPrediction(){
  ...
  for (int ilep = 0; ilep < NLep; ilep++){
	...
	srCorrErr[iSample] = bkg->srBkgCorrh[iSample-1]->GetBinError(1);
      ...
      cout<<sr[0]<<"\t";
      cout<<b<<"+-"<<TotUnc<<"("<<TotUncPercent<<")"<<endl;
